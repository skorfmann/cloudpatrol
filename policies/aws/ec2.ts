...
import { Policy, PolicyInterface } from '../../lib/policy';
import { Reportable } from '../../lib/reporter';
 ...
 */
export class Ec2InstanceTypePolicy extends Policy implements PolicyInterface {  
  ...
  public validator(node: ec2.CfnInstance, reporter: Reportable): void { 
  ...
  private instanceTypes(): string[] {
    if (Array.isArray(this.instanceClasses)) {      
      return this.instanceClasses.flatMap(c => ((this.instanceSizes && this.instanceSizes.map(s => `${c}.${s}`) || [`${c}.`])))
